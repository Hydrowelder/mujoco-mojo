...
class TextureBase(XMLModel, ABC):
    ...
    """If the file attribute is specified, then this sets the Media Type (formerly known as MIME types) of the file to be loaded. Any filename extensions will be ignored. Currently image/png and image/ktx are supported."""
    ...
    gridsize: tuple[int, int] | None = (1, 1)
...
class Texture(TextureBase):
    ...
    is_builtin: Literal[False] = False


class TextureBuiltIn(TextureBase):
    """
    This element creates a texture asset, which is then referenced from a material asset, which is finally referenced from a model element that needs to be textured.
...
AnyTexture = Annotated[
    Texture | TextureBuiltIn,
    Field(discriminator="is_builtin"),
]
