...
class Integrator(StrEnum):
    ...
    For this method, D only includes derivatives of joint damping. Note that in this case D is diagonal and Mhat is symmetric, so LT L decomposition (a variant of Cholesky) can be used. This factorization is stored mjData.qLD. If the model has no joint damping or the eulerdamp disable-flag is set, implicit damping is disabled and the semi-implicit update (3) is used, rather than (6), avoiding the additional factorization of Mhat (additional because M is already factorized for the acceleration update (5))."""
    ...
    For this method, D includes derivatives of all forces used in the implicit method, with the exception of the centripetal and Coriolis forces c(v) computed by the RNE algorithm. Additionally, it is symmetrized D←(D+DT)/2. One reason for dropping the RNE derivatives is that they are the most expensive to compute. Second, these forces change rapidly only at high rotational velocities of complex pendula and spinning bodies, scenarios which are not common and already well-handled by the Runge-Kutta integrator (see below). Because the RNE derivatives are also the main source of asymmetry of D, by dropping them and symmetrizing, we can use the faster LTL rather than LU decomposition.
    """

    DISCRETE = "discrete"
    """The integrators above preserve MuJoCo's continuous-time contract: forward dynamics compute the instantaneous acceleration, and integration is a separate stage which advances the state. This integrator gives up that separation, in the tradition of the velocity-stepping schemes: the constraint solve and the implicit update are one operation whose solution is the step itself, so integration is trivial and the additional factorization of `Mhat` performed by the integrators above (stored in `mjData.qH` or `mjData.qLU`) is not needed. The solver minimizes the same convex Gauss-principle objective, but in the effective metric `Mhat = M + hD + h^2K * M`, where `D` and `K` denote positive damping and stiffness matrices: the negated derivatives of the smooth forces, restricted to terms which keep `Mhat` positive definite. Constraint forces are therefore computed against the same effective inertia which performs the velocity update.

    The constraint rows are treated implicitly in the same way: each row's solref spring-damper is evaluated at the end of the step and folded into the row's impedance and reference. Constraint rows are then stable for any solref at any timestep. A contact or limit row whose spring is stiffer than the timestep can resolve (`timeconst * dampratio < h`) would however rebound on impact, so under this integrator the refsafe flag replaces such rows by the resolved row: the stiffest zero-restitution spring for the timestep at the authored damping ratio. Rows with `timeconst * dampratio >= h` and equality rows are left as authored; with refsafe disabled, every row is honored and timeconst -> 0 approaches the rigid-constraint limit (the violation is removed in one step) rather than an instability. The row's effective impedance is capped at the maximum impedance `mjMAXIMP`, which keeps the constraint weights well-conditioned. The cost is extra damping of the constraint response, growing with h/timeconsth/timeconst; the specified damping ratio is recovered as h -> 0.

    The `h^2 * K` term makes `discrete` the only integrator which is implicit in position: joint, tendon and flex stiffness and actuator position feedback are stable at timesteps far beyond the explicit stability limit `h ~< 2 / omega_max`, where `omega_max` is the highest natural frequency in the model. The terms entering `D` and `K` are: joint damping and stiffness, tendon damping and stiffness, actuator gains (clamped to the stabilizing sign), flex elasticity and damping, and the dissipative drag component of fluid forces. Terms which fit inside the sparsity pattern of `M` (the joint diagonals, the fluid drag blocks) join its tree-structured factorization with no fill-in and add no cost to the solve; the couplings introduced by tendon, actuator and flex terms are applied matrix-free inside the iterative solvers. This split determines solver support: CG is matrix-free and requires only products with `Mhat`, and Newton merges the metric directly into its Hessian, while the dual `PGS` and noslip solvers require an explicit factorization.

    Under PGS, tendon and actuator terms are therefore excluded from the metric and their forces integrate explicitly, like the sparsity-pattern restriction of `implicit` above. Noslip post-processing under a primal solver splits the metric: the main solve runs in the full `Mhat`, while the post-pass consumes the factored part alone as an approximation. Flex terms, which are too stiff to integrate explicitly, raise an error. The Coriolis and centripetal derivatives are excluded — they cannot enter a positive-definite metric — so gyroscopically-dominated mechanisms are better served by `implicit`; standalone free bodies receive the same local gyroscopic treatment as `implicitfast`.

    Under this integrator forward dynamics computes the discrete step map: `mjData.qacc` holds the velocity difference `(v+ - v) / h` rather than the continuous-time acceleration, so the same state yields a different `qacc` under a different timestep; as h -> 0 it converges to the continuous acceleration. Acceleration-stage sensors (accelerometer, force/torque) report this finite-difference acceleration, which is what a physical sensor averaging over one timestep measures. Inverse dynamics inverts the same relation, so the fwdinv consistency check remains meaningful and invdiscrete is implied. Because `qacc` is the step, derivatives of forward dynamics are derivatives of the discrete transition, with no discretization gap for gradient-based methods."""
