  ...
  function monitor() {
    return {
      ...
      refreshStats() {
        ...
        const pctOfTotal = (n) => (n / n_trial * 100).toFixed(1);
        ...
        this.stats = [
          ...
          },
          {
            ...
            tooltip: "Trials that ran but failed one or more requirement checks, including early terminations triggered by a failing requirement. Not runtime errors."
          },
          {
            label: "Errors",
            ...
            tooltip: "Trials that raised an unhandled runtime error while being processed. These runs broke; they were never judged against requirements."
          },
          {
            label: "Time Elapsed",
          ...
          },
          {
            ...
            tooltip: "Trials that have not finished yet. Throughput is derived from the average duration of completed trials (successes and failures; errors excluded) and the number of processors."
          },
          {
            label: this.status.is_complete ? "Finished" : "Est. Remaining",
            value: this.status.is_complete ? "00:00:00" : this.status.time_remaining,
            color: "text-slate-500",
            subValue: this.status.is_complete ? `Finished: ${this.status.end_time}` : `ETA: ${this.status.end_time}`,
            tooltip: this.status.is_complete ? "The job has finished. Shows the local time at which the job completed." : "Estimated wall-clock time until all trials finish, based on the average completed-trial duration (errors excluded). ETA is the projected local completion time."
          }
        ];
      },
      ...
      progressGradient() {
        const segs = [
          { color: "#10b981", n: this.status.n_success },
          // emerald-500
          { color: "#f43f5e", n: this.status.n_failed },
          // rose-500
          { color: "#f59e0b", n: this.status.n_error }
          // amber-500
        ].filter((seg) => seg.n > 0);
      ...
      renderTimeline() {
        const bins = this.status.timeline ?? [];
        ...
        const layout = {
          ...
          xaxis: {
            ...
            showspikes: true,
            spikemode: "across",
            spikelinecolor: "#06b6d4",
            // cyan-500, same as the trial viewer
            spikethickness: -2
          },
          yaxis: { gridcolor: gridColor, title: { text: "trials" } },
          legend: narrow ? { orientation: "h", x: 0.5, xanchor: "center", y: -0.45, yanchor: "top", font: { size: 10 } } : { orientation: "h", y: 1.15, x: 0 },
          hovermode: "x unified",
          hoverlabel: {
