...
Please report this to https://github.com/markedjs/marked.`, e) {
  ...
  function sensai() {
    return {
      ...
      async send() {
        ...
        this._scrollToBottom();
        try {
          const history = this.messages.slice(0, -1).slice(-20).map((m2) => ({ role: m2.role, content: m2.content }));
          ...
          if (trimmed.startsWith("event: ")) {
            ...
            try {
              ...
              if (eventType === "text_delta" && typeof payload.delta === "string") {
                ...
                const content = typeof payload.message === "string" ? payload.message || this.streamingContent || "(no response)" : this.streamingContent || "(no response)";
                this.messages.push({
                  role: "assistant",
                  content,
      ...
      _loadMessages() {
        try {
          ...
          if (stored) {
            ...
            this.messages = parsed.map((m2) => ({ ...m2, ts: m2.ts ?? Date.now() }));
          }
        } catch {
        }
      },
  ...
  function downloadSensAIHistory() {
    const stored = localStorage.getItem(STORAGE_KEY);
    let messages = [];
    try {
    ...
    const payload = {
      ...
      messages: messages.map((m2) => {
        const entry = {
          timestamp: new Date(m2.ts).toISOString(),
          sender: m2.role === "user" ? "user" : "sensai",
          message: m2.content
        };
  ...
  function initSensAICodeBlocks(container) {
    try {
  ...
  function sensaiMsgData(role) {
    return {
      ...
      measure() {
        const self = this;
        const content = self.$el.querySelector(".sensai-md") ?? self.$el.querySelector("p");
