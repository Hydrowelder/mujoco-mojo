  ...
  return {
    ...
    async init() {
      ...
      try {
        ...
        if (data && !data.error) {
          (Alpine.store('dojo') as DojoStore).updateSync(Date.now(), data.is_complete);
        ...
        store.setPageReady(true, (this.status as JobStatus & { is_complete?: boolean }).is_complete);
      }
    },

    handleDataUpdate(data: JobStatus) {
      const wasInit = this.hasInitialData;
      const prev = { ...this.prevStatus } as { n_done: number; n_success: number; n_failed: number };
    ...
    getHolidayTheme(): HolidayTheme {
      ...
      if ((m === 11 && d === 31) || (m === 0 && d <= 2)) {
        return { name: 'New Year', emojis: ['🎆', '✨', '🥂'], colors: ['#ffcc00', '#ffffff'] };
      }
      if (m === 2 && d === 14) {
        return { name: 'Pi Day', emojis: ['π', '🥧'], colors: ['#ff9900'] };
      }
      if (m === 2 && d === 17) {
        return { name: "St. Patrick's Day", emojis: ['🍀', '🌈'], colors: ['#22c55e', '#166534'] };
      }
      if (m === 4 && d === 4) {
        return { name: 'May the 4th', emojis: ['⚔️', '🌌', '✨'], colors: ['#FFE81F', '#2dd4bf'] };
      }
      if ((m === 9 && d >= 25) || (m === 10 && d === 1)) {
        return { name: 'Halloween', emojis: ['🎃', '👻', '🦇'], colors: ['#ff6600', '#9437ff'] };
      }
      if (m === 11 || (m === 0 && d <= 15)) {
        return { name: 'Winter Snow', emojis: ['❄️', '⛄', '🌨️'], isSnow: true };
      }
