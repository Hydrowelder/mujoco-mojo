...
!!! warning "Default Values"
    Some attributes in this package make use of default values. Wherever possible, the default values match what is stated in the [XML Reference](https://mujoco.readthedocs.io/en/stable/XMLreference.html#xml-reference){:target="_blank"}. The `to_xml` method found in all `XMLModel` (which is most) has an argument which will omit serializing fields set as default.
